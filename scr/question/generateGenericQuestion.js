...
function generateGenericQuestion() {
    ...
    Từ năm ${startYear} đến năm ${endYear}, ${description} (gọi là y) được tính theo công thức y = at + b, trong đó y là ${description}, t là số năm tính từ năm ${startYear}. Biết rằng vào năm ${startYear}, ${description} là ${initialValue}${unit}, và vào năm ${endYear}, ${description} là ${finalValue}${unit}.

    a) Hãy lập công thức liên hệ giữa ${description} và số năm. Dựa vào công thức, tính ${description} cho năm {year}.

    b) Nếu vào năm ${endYear + 1}, ${description} là ${finalValue + (finalValue - initialValue)} ${unit}. Hãy tính số năm cần để đạt được ${description} là ${finalValue + (finalValue - initialValue) + 1} ${unit}. Làm tròn kết quả đến số thập phân thứ 2.
    `;

    const newQuestion = initialQuestion.replace(/\{year\}/g, startYear + Math.floor((endYear - startYear) / 2));

    console.log("Đề bài mới:");
    console.log(newQuestion);

    const a = (finalValue - initialValue) / (endYear - startYear);
    const b = initialValue - a * startYear;
    console.log("a =", a);
    console.log("b =", b);

    const midYearValue = a * (startYear + Math.floor((endYear - startYear) / 2)) + b;
    console.log(`${description.toLowerCase()} cho năm ${startYear + Math.floor((endYear - startYear) / 2)}:`, midYearValue);

    const targetValue = finalValue + (finalValue - initialValue) + 1;
    const yearsToReachTarget = (targetValue - b) / a;
    console.log(`Số năm cần để đạt được ${description.toLowerCase()} là ${targetValue}${unit}:`, yearsToReachTarget.toFixed(2));

    return nQ = initialQuestion.replace(/\{year\}/g, startYear + Math.floor((endYear - startYear) / 2));
}
module.exports = {
